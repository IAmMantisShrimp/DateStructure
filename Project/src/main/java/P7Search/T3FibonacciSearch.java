 ...
 * @description: 斐波那契查找
 * @author: HuaRunSheng
 * @create: 2022-01-03 15:30
 *
 * 总结:与二分法折半查找相比，斐波那契查找的优点是它只涉及加法和减法运算，而不用除法，
 * 而除法比加减法要占用更多的时间，因此，斐波那契查找的运行时间理论上比折半查找小，但是具体还是得视具体情况而定。
 *      推荐使用插值查找,找mid效率较高
 **/

public class T3FibonacciSearch {
    ...
    public static int fibSearch(int[] arr, int key) {
        int low = 0;//即left
        int high = arr.length - 1;//即right
        int k = 0; //表示斐波那契分割数值的下标
        int mid = 0; //存放 mid 值
        int f[] = fib(); //获取到斐波那契数列
        //获取到斐波那契分割数值的下标 (F[k]-1) = (F[k-1]-1) + (F[k-2]-1) +1
        while (high > f[k] - 1) {
            k++;
        }
        // 因为 f[k] 值 可能大于 arr 的 长度，因此我们需要使用 Arrays 类，构造一个新的数组，并指向 temp[]
        // 不足的部分会使用 0 填充
